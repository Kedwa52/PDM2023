...
// somecode var to let the pos be more

function setup() {
  createCanvas(400, 400);
  characterOne = new Character(100,100,50);
  characterTwo = new Character (300,300,80);
}
...
class Character {
  ...
  mousePressed(){
    //calculate mouse position, is it within the square
  ...
  mouseDragged(){
    if (this.dragging){
    this.x = this.characterStartX + (this.mouseX - this.dragStartX)
    this.y = this.characterStartY + (this.mouseY - this.dragStartX)
    }
  }
  mouseReleased(){
    if (this.dragging)
