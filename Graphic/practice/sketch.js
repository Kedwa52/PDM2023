...
let game = { score: 0, maxScore: 0, maxTime: 45 , elapsedTime: 0, totalSprites: 15, state: GameState.Start, targetSprite: 2 };

function preload() {
  spriteSheet = loadImage("assets/SpelunkyMonk.png");
}
...
function reset() {
  ...
  for(let i=0; i < game.totalSprites; i++) {
    animations[i] = new WalkingAnimation(spriteSheet,80,80,random(100,300),random(100,300),9,random(0.5,1),6,random([0,1]));
  }
}
      ...
      for (let i=0; i < animations.length; i++) {
        ...
        if (contains) {
          if (animations[i].moving != 0) {
            animations[i].stop();
            if (animations[i].spritesheet === spritesheet[game.targetSprite])
              game.score += 1;
            else
              game.score -= 1;
          }
